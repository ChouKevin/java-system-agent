...
class ApiRouteControllerTest {
    ...
    void should_return_multiple_candidates_with_per_candidate_revisions() throws Exception {
        given(apiRouteApplicationService.lookup(
    ...
    void should_return_ok_with_empty_candidate_list() throws Exception {
        given(apiRouteApplicationService.lookup("/missing", Optional.empty(), Optional.empty()))
                .willReturn(List.of());
    ...
    void should_accept_suggest_limit_boundary(int limit) throws Exception {
        given(apiRouteApplicationService.suggest(
                "/orders", Optional.empty(), Optional.empty(), limit)).willReturn(List.of());
        ...
        mockMvc.perform(post("/v1/api-routes/suggest")
                        ...
                        .content("{\"apiPath\":\"/orders\",\"limit\":" + limit + "}"))
                .andExpect(status().isOk());
        then(apiRouteApplicationService).should().suggest(
    ...
    void should_forward_lookup_scope_and_method() throws Exception {
        ...
        given(apiRouteApplicationService.lookup("/orders/42", Optional.of("POST"), Optional.of(scope)))
                .willReturn(List.of());
        ...
        mockMvc.perform(post("/v1/api-routes/lookup")
                ...
                .andExpect(status().isOk());
        then(apiRouteApplicationService).should().lookup(
