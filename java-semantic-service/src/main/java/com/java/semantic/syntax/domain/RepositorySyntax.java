 ...
 * @param sourceTypes 所有型別的 metadata
...
public record RepositorySyntax(
    ...
    public RepositorySyntax(
            ...
            List<SourceTypeMetadata> sourceTypes,
            List<SourceExtractionOutcome> extractionOutcomes) {
        this(entryPoints, sourceTypes, extractionOutcomes, Optional.empty());
    }

    /** 保留既有 callers 的兩個集合建構契約 */
    public RepositorySyntax(List<EntryPointClass> entryPoints, List<SourceTypeMetadata> sourceTypes) {
        this(entryPoints, sourceTypes, List.of(), Optional.empty());
    }
