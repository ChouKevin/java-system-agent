 ...
 * 公開呼叫點的完整半開區間。
 *
 * <p>sourceFile 為服務端已解析的工作區相對來源檔路徑。行與字元座標皆為一基；
...
public record CallSiteRange(
    ...
    public CallSiteRange {
        ...
        Assert.isTrue(startLine > 0, "startLine must be positive");
        Assert.isTrue(startCharacter > 0, "startCharacter must be positive");
        Assert.isTrue(endLine > 0, "endLine must be positive");
        Assert.isTrue(endCharacter > 0, "endCharacter must be positive");
