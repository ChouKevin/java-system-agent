...
/** Complete normalized response-local graph fragment discovered through incoming call hierarchy queries. */
public record IncomingGraphFragment(
    ...
    public IncomingGraphFragment {
        ...
        if (GraphAnalysisStatus.SUCCESS.equals(status)) {
            Assert.isTrue(CollectionUtils.isEmpty(warnings) && CollectionUtils.isEmpty(errors),
