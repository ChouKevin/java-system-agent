...
class AgentActionPromptRendererTest {
    ...
    void labels_evidence_with_the_capability_recorded_in_query_history() {
        ...
        assertThat(AgentActionPromptRenderer.SYSTEM_INSTRUCTION)
                ...
                .contains("A METHOD concept issues only source and call-graph navigation")
                .contains("discover the abstract or interface declaration TYPE named in the method contract")
                .contains("choose codebase_discover_method_implementations from the eligible abstract or interface method member")
                .contains("discover its owning TYPE, inspect FIELD members, and choose the field's codebase_find_internal_references follow-up")
                .contains("execute the requested capability once for every distinct issued item")
                .contains("obtain implementation evidence first and read source from the selected implementation candidate")
                .contains("execute an already issued exact provider-bound FOLLOW_UP for a still-missing evidence type")
                .contains("Do not repeat the same discovery execution payload when it did not issue the required targetCapability")
                .contains("change the concept kind or search term, or choose an eligible current FOLLOW_UP")
