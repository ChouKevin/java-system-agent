...
class ApiWriteProtectionFilterTest {
    ...
    private MockMvc buildMockMvc(String configuredToken) {
        ...
        when(gitService.pullRepository("test-repo")).thenReturn("pulled");
        when(analysisService.allRepos()).thenReturn(List.of());
        when(analysisService.reloadRepoAfter(eq("test-repo"), any()))
    ...
    void should_return_403_when_write_token_is_not_configured() throws Exception {
        ...
        mockMvc.perform(post("/git/pull-repo/test-repo")
                ...
                .andExpect(jsonPath("$.path").value("/git/pull-repo/test-repo"))
