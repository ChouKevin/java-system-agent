...
public final class IncomingCallGraphExecutor implements CapabilityExecutor {
    ...
    public CapabilityExecutionResult execute(CapabilityInvocation invocation) {
        ...
        return adapter.incomingCallGraph(invocation);
    }
    ...
    private void requireExactCapability(CapabilityInvocation invocation) {
        ...
        if (!capability.equals(invocation.capability())) {
            throw new CapabilityExecutionContractException("capability invocation does not match incoming-call-graph executor");
        }
    }
}
