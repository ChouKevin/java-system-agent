...
public class CallGraphExpandTools {
    ...
    @Tool(name = ToolNames.FIND_CALL_GRAPH,
          description = "Expand a truncated or data-access call graph node and return structured analysis status.")
    public AnalysisResult<FlattenedCallGraph> findCallGraph(
        ...
        try {
            return analysisService.analyzeMethodStructured(repoId, packageName, className, methodSignature);
        } catch (Exception e) {
