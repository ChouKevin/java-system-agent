...
public final class BoundedAnalysisCoordinator implements ExecuteAnalysisUseCase {
    ...
    public AnalysisExecutionResult execute(AnalysisExecutionCommand command) {
        ...
        try {
            lifecycle = attemptLifecycleManager.start(command);
        ...
        while (lifecycle.run().outcome().isEmpty()) {
            ...
            GoalEvaluation goalEvaluation = goalEvaluator.evaluate(command.goal(), lifecycle.state(), Optional.empty());
            ...
            boolean retryAllowed = semanticRetryPolicy.shouldRetry(initialResult, 1, lifecycle.state().budget());
            SemanticStepResult semanticStep = semanticResultHandler.handle(
            ...
            if (semanticStep.disposition() == SemanticStepDisposition.RETRYABLE && retryAllowed) {
                ...
                semanticStep = semanticResultHandler.handle(lifecycle.state(), query, retryResult, false);
                lifecycle = withState(lifecycle, semanticStep.state());
            }

            DiscoveryPinResult discoveryPinResult;
            try {
                discoveryPinResult = pinDiscoveries(lifecycle, semanticStep.newDiscoveries());
            } catch (AttemptPreparationException exception) {
                return concludeForPreparationFailure(exception);
            }
            lifecycle = discoveryPinResult.lifecycle();
            if (discoveryPinResult.cancelled()) {
                return conclude(lifecycle, AttemptOutcome.CANCELLED, AnalysisOutcome.CANCELLED,
                        AnalysisTerminationReason.CANCELLED);
            }
    ...
    private DiscoveryPinResult pinDiscoveries(
        ...
        for (RepositoryDiscovery discovery : sortedDiscoveries) {
            if (isCancellationRequested(current)) {
                return new DiscoveryPinResult(current, true);
            }
            if (current.state().repositoryScope().selection(discovery.repositoryId())
        ...
        return new DiscoveryPinResult(current, false);
    }
    ...
    private record DiscoveryPinResult(AttemptLifecycle lifecycle, boolean cancelled) {
