...
public final class GetEvidenceSourceExecutor implements CapabilityExecutor<GetEvidenceSourceExecutionInput> {
    ...
    public GetEvidenceSourceExecutor(JavaSemanticServiceHttpAdapter adapter) {
        this.adapter = Objects.requireNonNull(adapter, "Java Semantic Service adapter must not be null");
    }

    @Override
    public CapabilityExecutionResult execute(CapabilityExecutionContext context, GetEvidenceSourceExecutionInput input) {
        return adapter.getEvidenceSource(context, input);
    }
}
