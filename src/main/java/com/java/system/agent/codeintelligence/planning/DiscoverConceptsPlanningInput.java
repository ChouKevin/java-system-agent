...
public record DiscoverConceptsPlanningInput(
        ...
        @JsonProperty(required = true) @NotEmpty @Size(max = 4) List<@Valid @NotNull Term> terms,
    ...
    public enum MatchMode { TOKEN_EXACT, TOKEN_PREFIX }

    public enum Kind {
        TYPE, METHOD, FIELD, ANNOTATION_USAGE, TYPE_USAGE, API_ROUTE, MQ_DESTINATION, SCHEDULE,
        MAPPER_STATEMENT, SQL_IDENTIFIER, CONFIGURATION_KEY, OUTBOUND_API, MQ_PUBLISHER,
        ERROR_CONTRACT, ENUM_CONSTANT
    }

    public record Term(@NotBlank @Size(min = 2, max = 128) String value, @NotNull MatchMode matchMode) {
    }
}
