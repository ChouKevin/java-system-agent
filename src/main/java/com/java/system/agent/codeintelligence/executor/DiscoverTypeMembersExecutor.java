...
public final class DiscoverTypeMembersExecutor implements CapabilityExecutor<DiscoverTypeMembersExecutionInput> {
    ...
    public DiscoverTypeMembersExecutor(JavaSemanticServiceHttpAdapter adapter) {
        this.adapter = Objects.requireNonNull(adapter, "Java Semantic Service adapter must not be null");
    }

    @Override
    public CapabilityExecutionResult execute(CapabilityExecutionContext context, DiscoverTypeMembersExecutionInput input) {
        return adapter.discoverTypeMembers(context, input);
    }
}
