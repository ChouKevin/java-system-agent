...
public final class ResolveConceptExecutor implements CapabilityExecutor<ResolveConceptExecutionInput> {
    ...
    public ResolveConceptExecutor(JavaSemanticServiceHttpAdapter adapter) { this.adapter = Objects.requireNonNull(adapter, "Java Semantic Service adapter must not be null"); }
    @Override public CapabilityExecutionResult execute(CapabilityExecutionContext context, ResolveConceptExecutionInput input) { return adapter.resolveConcept(context, input); }
}
