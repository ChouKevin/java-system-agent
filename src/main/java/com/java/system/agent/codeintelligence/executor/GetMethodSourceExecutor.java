...
public final class GetMethodSourceExecutor implements CapabilityExecutor<GetMethodSourceExecutionInput> {
    ...
    public GetMethodSourceExecutor(JavaSemanticServiceHttpAdapter adapter) { this.adapter = Objects.requireNonNull(adapter, "Java Semantic Service adapter must not be null"); }
    @Override public CapabilityExecutionResult execute(CapabilityExecutionContext context, GetMethodSourceExecutionInput input) { return adapter.getMethodSource(context, input); }
}
