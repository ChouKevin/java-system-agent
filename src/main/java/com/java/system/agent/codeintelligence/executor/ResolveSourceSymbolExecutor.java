...
public final class ResolveSourceSymbolExecutor implements CapabilityExecutor<ResolveSourceSymbolExecutionInput> {
    ...
    public ResolveSourceSymbolExecutor(JavaSemanticServiceHttpAdapter adapter) {
        this.adapter = Objects.requireNonNull(adapter, "Java Semantic Service adapter must not be null");
    }

    @Override
    public CapabilityExecutionResult execute(CapabilityExecutionContext context, ResolveSourceSymbolExecutionInput input) {
        return adapter.resolveSourceSymbol(context, input);
    }
}
