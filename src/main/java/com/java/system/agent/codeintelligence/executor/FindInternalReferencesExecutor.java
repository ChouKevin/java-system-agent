...
public final class FindInternalReferencesExecutor implements CapabilityExecutor<FindInternalReferencesExecutionInput> {
    ...
    public FindInternalReferencesExecutor(JavaSemanticServiceHttpAdapter adapter) {
        this.adapter = Objects.requireNonNull(adapter, "Java Semantic Service adapter must not be null");
    }

    @Override
    public CapabilityExecutionResult execute(CapabilityExecutionContext context, FindInternalReferencesExecutionInput input) {
        return adapter.findInternalReferences(context, input);
    }
}
